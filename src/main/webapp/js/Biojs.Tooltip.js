...
/** @lends Biojs.Tooltip# */
{
  ...
  */
  opt: {
     ...
     cbRender: undefined,
	 arrowType: "left_top",
	 showOnEvent: 'mousemove',
	 hideOnEvent: 'mouseexit',
  ...
  _initialize: function( ) {
      ...
      if ( refPos == Biojs.Tooltip.MOUSE_POSITION ) {
    	  targetSelector.bind( this.opt.showOnEvent, function (e) {
        	  self._showUp(e, cbRender);
          
          }).bind( this.opt.hideOnEvent, function() {
              timer = setTimeout( 'Biojs.getInstance(' + self.getId()  + ')._hide()' , self.opt.delay );
          });
    	  
      } else if ( refPos == Biojs.Tooltip.MOUSE_ELEMENT_POSITION ) {
    	  targetSelector.bind( this.opt.showOnEvent, function (e) {  			
    		  self._showUp(e, cbRender);
  ...
  _showUp: function(e,cbRender) {
      ...
      // Set the content
      content = cbRender.call( this, e.target );
      ...
      // Positioning 
      var pos = this._pointerEventToXY(e);
      ...
      // Event triggering 
      this.raiseEvent( Biojs.Tooltip.EVT_ON_SHOW_UP, { 'target': target });
  },
  
  _pointerEventToXY: function(e){
      var out = {x:0, y:0};
